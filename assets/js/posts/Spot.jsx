...
const Spot = ({ onlineUser }) => {
    ...
    const fetchData = async () => {
        ...
        setError(null);
        try {
            const response = await instance.get(spotsUserEndPoint);
            const spotsData = response.data.member;
            setSpotsUserList(spotsData);
        } catch (error) {
            console.error(error);
    ...
    useEffect(() => {
        fetchData(spotsUserEndPoint);
    }, []);

    //console.log("spotsUserList :", spotsUserList);
