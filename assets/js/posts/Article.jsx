...
const Article = ({ onlineUser }) => {

    const [selectedItem, setSelectedItem] = useState(null);
    const [articlesUserList, setArticlesUserList] = useState([]);
    ...
    const fetchData = async () => {
        ...
        setError(null);
        try {
            const response = await instance.get(articlesUserEndPoint);
            const articlesData = response.data.member;
            setArticlesUserList(articlesData);
        } catch (error) {
            console.error(error);
    ...
    const articlesUserEndPoint = "/articles?page=1&user="+userId;

    useEffect(() => {
        fetchData(articlesUserEndPoint);
    }, []);

    //console.log("articlesUserList :", articlesUserList);

    const articles = articlesUserList;
