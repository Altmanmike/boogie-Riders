...
const Event = ({ onlineUser }) => {
    
    const [selectedItem, setSelectedItem] = useState(null);
    const [eventsUserList, setEventsUserList] = useState([]);    
    ...
    const fetchData = async () => {
        ...
        setError(null);
        try {
            const response = await instance.get(eventsUserEndPoint);
            const eventsData = response.data.member;
            setEventsUserList(eventsData);            
        } catch (error) {
            console.error(error);
    ...
    const eventsUserEndPoint = "/events?page=1&user=" + userId;
    
    useEffect(() => {              
        fetchData(eventsUserEndPoint);        
    }, []);

    //console.log("eventsUserList :", eventsUserList);

    const events = eventsUserList;       
