...
const EventsFeed = () => {
    ...
    const [eventsFeedList, setEventsFeedList] = useState([]);    
    ...
    const fetchData = async () => {
        ...
        setError(null);
        try {
            const response = await instance.get(eventsEndPoint);
            const eventsData = response.data.member;
            setEventsFeedList(eventsData);            
        } catch (error) {
            console.error(error);
    ...
    const eventsEndPoint = "/events?page=1";    
    ...
    const eventsFeed = eventsFeedList;   
