...
const SpotsFeed = () => {
    ...
    const [spotsFeedList, setSpotsFeedList] = useState([]);    
    ...
    const fetchData = async () => {
        ...
        setError(null);
        try {
            const response = await instance.get(spotsEndPoint);
            const spotsData = response.data.member;
            setSpotsFeedList(spotsData);            
        } catch (error) {
            console.error(error);
    ...
    const spotsEndPoint = "/spots?page=1";    

    useEffect(() => {              
        fetchData(spotsEndPoint);        
    }, []);

    //console.log("spotsFeedList :", spotsFeedList[0].member);
    const spotsFeed = spotsFeedList;    
